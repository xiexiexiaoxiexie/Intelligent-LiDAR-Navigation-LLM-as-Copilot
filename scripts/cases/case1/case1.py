...
with open('./src/osmAG_intelligent_navigation/scripts/cases/case1/case1.json', 'r') as config_file:
...
def prepare(areas,case_num):
    ...
    args['world_file']=case1_config['case1']['world_file']
    return args
...
def navigation_result_callback(data,case_config,pub):
    ...
    over_lay_text_manager.add_text_box('/human_instruction', '', 600, 60, 10, 10, 12, 2, ColorRGBA(0.0, 0.84, 1.0, 1.0), ColorRGBA(0.0, 0.0, 0.0, 0.7))
    over_lay_text_manager.set_text(f"{case_config['human_natural_language_command']}",0)
    over_lay_text_manager.publish_text(0)
    list_free_passes=list(case_config['free_passes'].values())
    print(f"{utility.PURPLE}[case1.py]navigation_result_callback: {data.data}, only send next task when result is true., human instruction is {case_config['human_natural_language_command']}{utility.RESET}")
    # in gazebo simulation, all test case is eventually accessible, therefore only start next trial when result is true
    if data.data=='True' or data.data=='Parameter_ready':
        if data.data=='True':
            clear_costmaps()
        pub.publish(StringList(data=list_free_passes[free_pass_index]))
        print(f"{utility.PURPLE}[case1.py]:publishing the {free_pass_index} trial: {StringList(data=list_free_passes[free_pass_index])}{utility.RESET}")
        free_pass_index+=1
    time.sleep(1)
    over_lay_text_manager.publish_text(0)
...
if __name__ == "__main__":
    ...
    case_config=case1_config['case1']
    ...
    rospy.set_param('/second_floor', False)
