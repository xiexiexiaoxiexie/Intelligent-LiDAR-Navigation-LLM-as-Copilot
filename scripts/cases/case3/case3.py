...
with open('./src/osmAG_intelligent_navigation/scripts/cases/case3/case3.json', 'r') as config_file:
    case3_config = json.load(config_file)
...
def prepare(areas,case_num):
    free_pass=case3_config[case_num]['free_passes']['free_pass1']
    args=prepare_arg(areas,free_pass[0],case_num)
    args['free_pass_start']=free_pass[0]
    args['free_pass_end']=free_pass[1]
    args['human_natural_language_command']=case3_config['case3']['human_natural_language_command']
    args['world_file']=case3_config['case3']['world_file']
    return args
...
def navigation_result_callback(data,case_config,pub):
    ...
    print(f"{utility.PURPLE}[case3.py]navigation_result_callback: {data.data}, only send next task when result is true.{utility.RESET}")
    if data.data=='True' or data.data=='Parameter_ready':
        if data.data=='True':
            clear_costmaps()
        pub.publish(StringList(data=list_free_passes[free_pass_index]))
        print(f"{utility.PURPLE}[case3.py]:publishing the {free_pass_index} trial: {StringList(data=list_free_passes[free_pass_index])}{utility.RESET}")
        free_pass_index+=1
...
if __name__ == "__main__":
    ...
    case_config=case3_config['case3']
