...
def prepare(areas,case_num):
    free_pass=case2_config[case_num]['free_passes']['free_pass1']
    args=prepare_arg(areas,free_pass[0],case_num)
    args['free_pass_start']=free_pass[0]
    args['free_pass_end']=free_pass[1]
    args['human_natural_language_command']=case2_config['case2']['human_natural_language_command']
    args['world_file']=case2_config['case2']['world_file']
    return args
...
def navigation_result_callback(data,case_config,pub):
    ...
    print(f"{utility.PURPLE}[case2.py]navigation_result_callback: {data.data}, only send next task when result is true.{utility.RESET}")
    if data.data=='True' or data.data=='Parameter_ready':
        if data.data=='True':
            clear_costmaps()
        pub.publish(StringList(data=list_free_passes[free_pass_index]))
        print(f"{utility.PURPLE}[case2.py]:publishing the {free_pass_index} trial: {StringList(data=list_free_passes[free_pass_index])}{utility.RESET}")
        free_pass_index+=1
    time.sleep(1)
    over_lay_text_manager.publish_text(0)
...
if __name__ == "__main__":
    ...
    case_config=case2_config['case2']
